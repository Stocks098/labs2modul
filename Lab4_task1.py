...
class human:
    ...
    def __init__(self, surname, name, age = None):
        ...
        Примеры:
        >>> human1 = human("Daniil", "Belkin", 19)    # Инициализация объекта класса
        """

        # Если введенное значение имени не является строкой, выдается ошибка
        if not(isinstance(name, str)):   raise TypeError("Имя введено не корректно")

        # Если введенная строка имеет символы, не являющиеся буквами, выдается ошибка
        if not (name.isalpha()): raise ValueError("Имя может содержать только буквы")

        # Если введенная строка начинается не с заглавной буквы, выдается ошибка
...
class student(human):
    ...
    def __init__(self, surname, name = None, age = None, number_of_debts = 0):
        ...
        Примеры:
        >>> student1 = student("Dannil", "Belkin", 20, 2)    # Инициализация объекта класса
        """
...
class prepod(human):
    ...
    def __init__(self, surname, name=None, age=None, number_of_student_expelled = 0):
        ...
        Примеры:
        >>> prepod1 = prepod("Vagurina", "Irina", 50, 3)    # Инициализация объекта класса
        """
